 ...
 * 
 * A candidate Chooser is an alogrithm that can find missing edges in the graph and follows a
 * certain strategy.
 * 
...
public abstract class ACandidateChooser {
  ...
  /**
   * Hook. This is the only method that should be overridden by subclasses. This should not be used
   * directly from the outside. Who ever uses a candidate chooser should call
   * {@link #getNextCandidates()}.
   * 
   * @return any available candidate pair.
   * @throws BattleFinishedException
   *           when no candidates are left. TODO should not only {@link #doGetNextCandidates()}
   *           throw this?
   */
  abstract Pair<File, File> doGetNextCandidates();

  /**
   * @return {@link Optional#empty()} if there are no more candidates in the graph. This means the
   *         battle is finished.
   */
