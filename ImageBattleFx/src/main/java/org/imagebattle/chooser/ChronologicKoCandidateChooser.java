...
public class ChronologicKoCandidateChooser extends ACandidateChooser {
  ...
  Pair<File, File> doGetNextCandidates() {
    ...
    while (currentLevel.size() < 2 && inDegree < vertexSet.size() - 1) {
      ...
      sortedFiles.stream()//
          .filter(file -> graph.inDegreeOf(file) == tempInDegree)// TODO what if there is only one
                                                                 // with zero losses?
    ...
    if (currentLevel.size() < 2) {
      // TODO checking if the battle is finished should be determined by edge count and implemented
      // in ACandidateChooser
