...
public class WinnerOrientedCandidateChooser extends ACandidateChooser {
	...
	Pair<File, File> doGetNextCandidates() {
		...
		// System.err.println();
		// System.err.println("last :" + pairToLossCountProduct.applyAsInt(candidates.get(calculatedCandidateCount -
		// 1)));
		...
		// Stream.generate(expRandomSupplier).limit(60).forEach(System.out::println); for testing the exponential
		// distribution

		log.trace("candidate pair count: {}   ", calculatedCandidateCount);

		Integer expRandomIndex = expRandomSupplier.get();
		expRandomIndex = expRandomIndex - 1; // most likely value is 1, 0 never apperas but we need 0 to access the
												// array
		...
		Pair<File, File> pair = candidatesStream//
				.sorted(Comparator.comparingInt(pairToLossCountProduct))// sorting is what takes the time => keep a
