...
public class WinnerOrientedCandidateChooser extends ACandidateChooser {
	...
	Pair<File, File> doGetNextCandidates() {
		...
		ToIntFunction<Pair<File, File>> pairToLossCountProduct = pair -> {
			...
			int loseCountKey = graph2.inDegreeOf(key);
			int loseCountValue = graph2.inDegreeOf(value);
			int loseSum = loseCountKey + loseCountValue; // use sum to not rate 0-8 as good as 0-0
			int loseDistance = Math.abs(loseCountKey - loseCountValue);// prefer 4-4 over 2-8 => low distance ist good

			int winCountKey = graph2.outDegreeOf(key);
			int winCountValue = graph2.outDegreeOf(value);
			int winSum = winCountKey + winCountValue; // use sum to not rate 0-8 as good as 0-0, a high winsum is good
			int winDistance = Math.abs(winCountKey - winCountValue);// prefer 4-4 over 2-8 => low distance ist good
		...
		// candidates.stream().limit(90).mapToInt(pairToLossCountProduct).forEach(i -> System.err.print(" " + i));
		// System.err.println();
		// System.err.println("last :" + pairToLossCountProduct.applyAsInt(candidates.get(calculatedCandidateCount - 1)));
		...
		Pair<File, File> pair = candidatesStream//
				.sorted(Comparator.comparingInt(pairToLossCountProduct))// sorting is what takes the time => keep a sorted list and adapt it to the changes ?
