...
public class ImageMachineLearning {
  ...
  private static void kMeans() {
    /*
     * How can kMeans be used for image battle? create clusters and rank the clusters?
     */
  ...
  private static Set<ResultListEntry> loadMostFightCountImages(long maxImageCount) {
    ...
    Function<? super ResultListEntry, ? extends Integer> entryToFightCount = entry -> entry.wins
        + entry.loses;
  ...
  private static void learnImageRating(long maxImageCount, int downsampleSize, int hiddenLayerCount,
      int iterationCount) {
    /*
     * idea 1 : binary classifier ( ignore or not? ) I think this is difficult. Don't do it yet.
     */
    ...
    LOG.info("imageDataSize: {}   controlDataSize: {}", imageDataList.size(),
        controlDataList.size());

    ImageMLDataSet trainingData = new ImageMLDataSet(downSampler, findBounds, downSampleHigh,
        downSampleLow);
    ImageMLDataSet controlData = new ImageMLDataSet(downSampler, findBounds, downSampleHigh,
        downSampleLow);
    ...
    IntStream.rangeClosed(1, iterationCount)//
        ...
        .forEach(iteration -> {
          ...
          for (MLDataPair pair : trainingData) {
            /*
             * FIXME why is output always 1 ? because weights don't change , because the gradients
             * are 0. Most cases output is 1 or -1. tanh(x) = 1 for x > 2 ... no gradient there ?!
             * => try sigmoid
             */
            MLData output = network.compute(pair.getInput());
            treeMap.put(dataToDouble.apply(pair.getIdeal()), dataToDouble.apply(output));
            // System.out.println(" Ideal=" + EncogUtility.formatNeuralData(pair.getIdeal()) + "
            // Actual="
          ...
          Stream<Entry<Double, Double>> last = treeMap.entrySet().stream()
              .skip(treeMap.size() - count);
    ...
    controlData.forEach(pair -> {
      ...
      String line = "ideal:" + dataToDouble.apply(ideal) + "   \tactual:"
          + dataToDouble.apply(actual) + "\n";
      sb.append(line);
    });
  ...
  private static Image read(File file, int readSize) {
    try {
      ...
      javafx.scene.image.Image image = new javafx.scene.image.Image(fis, readSize, readSize, true,
          true);
  ...
  private static IntToDoubleFunction createNormalizeForTanhFunction(int min, int max) {
    ...
    LOG.debug("max {}    min   {}     translate  {}     scale    {} ", max, min, translateValue,
        maxToMinRange);
  ...
  private static IntToDoubleFunction createNormalizeForSigmoidFunction(int min, int max) {
     ...
     * 
     * TRANSLATE min -> 0 = min -min max -> max - min diff -> diff - min
     * 
     * scale range diff / ( max - min) (diff) / example: max= 13 , min = -4 13 / ( 17) = 0,76, -4 /
     * 17 = -0,23 move range diff - ( max -min)/2 , aim for max-> 8,5 and min-> -8,5 => move= -4,5
     * example: 13 - 17/2 = 4,5 ; -4 - 17/2 = abs(max) - abs(min) /2 13 - 4 /2 = 9/2 = 4,5
     */
