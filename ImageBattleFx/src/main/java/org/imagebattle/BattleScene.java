...
/**
 * This is where two images are displayed next to each other and the user can click one of them. Then the next pair is displayed.
 * 
 * @author Besitzer
 *
 */
public class BattleScene extends Scene {
	...
	private BattleImageView imageViewLeft;
	private BattleImageView imageViewRight;
	...
	public BattleScene(StackPane switchSceneStackPane, ImageBattleFolder folder, Runnable switchSceneAction) {
		...
		MenuBar menuBar = new MenuBar(choosingMenu);
		...
		// this does not help: vBoxLeft.setMouseTransparent(true); // the vBox does not catch mouse events. Without it click on images would be catched by this.
		...
		Function<Supplier<Image>, Runnable> f = imageSupplier -> () -> {
			...
			children.remove(fullSizeImage);// avoid duplicate children exception, also left can replaced by right.
	...
	private void adaptImageSizes() {
		...
		// System.err.println("targetHeight:" + targetHeight + "   targetWidth:" + targetWidth);

		// TODO somehow this did not work correctly. fix or remove. Do we need two choosable layouting algorithms?
	...
	private void layoutEqualArea() {
		...
		if (windowWidth > 0 && windowHeight > 0) {
			...
			// somehow now images don't want to get wider than 715 and higher than 400
